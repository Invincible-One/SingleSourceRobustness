...
class ConfounderDataset(Dataset):
    ...
    def __init__(self, root, split, transform, target_transform, conditional_name, target_attr, confounder_attr):
        ...
        if isinstance(confounder_attr, list):
            ...
            self.confounder = [confounder_attr]
    ...
    def __getitem__(self, idx):
        ...
        #WARNING: conditional transformation could be implemented, e.g. transform(X, conditional)
...
class CelebA(ConfounderDataset):
    ...
    def __init__(
        ...
        self._data_folder = os.path.join(self.root, self._base_folder, "img_align_celeba")   #COMMENT: _data_folder is where data points are kept
        
        #COMMENT: split and select attrs, map values from {-1, 1} to {0, 1} and map multiple confounders to a single int
        ...
        confounder_arr = np.matmul(
                confounder_arr,
                np.power(2, np.arange(n_confounders)))
...
class CUB(ConfounderDataset):
    ...
    def __init__(
        ...
        #COMMENT: split. CUB only has one confounder.
